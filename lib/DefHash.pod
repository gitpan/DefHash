...
Another one that is currently used:

 alt.neg

This is used for example in summary of L<Rinci> function argument specification
to specify the negation. For example:
...
The C<alt.neg> version can be displayed, e.g. when showing summary for
command-line option C<--nofull> (while the "positive" summary can be used for
the C<--full> option).
...
This one means showing both positive and negative version in one statement, for
example:
 ...
 "full": {
     ...
     "summary": "Turn on full processing",
     "summary.alt.neg": "Turn off full processing"
     "summary.alt.posneg": "Turn on (off) full processing"
 }

In a CLI application, the C<alt.posneg> version can be used as a summary for
entry C<--full (--nofull)>.
