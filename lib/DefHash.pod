...
Another kind of alternate is grammatical variance based on noun numbers
(C<alt.numnoun.WHICH> where C<WHICH> can be C<singular> or C<plural>). Example
(in a Rinci function argument's specification, where an array argument like
C<file> translates in a CLI environment to multiple C<--file> option
specification):

 "file": {
     "schema": ["array*", {"of":"str*"}],
     "summary": "Specify one or more files to check",
     "summary.alt.numnoun.singular": "Add a file to check (can be specified multiple times)",
 }
